...
def create_graph(a:list[int]):
    ...
    for i in range(n, 0, -1):
        value = a[i-1]
        add_edge(s, 3*i-1, 0, 1)       # salida al duplicado para posiblemente empezar la secuencia por él o por uno igual
        add_edge(3*i-2, 3*i-1, 0, inf) # triplicado al duplicado para desde algun anterior que cumple modulo 7 con este pueda formar secuencia
        add_edge(3*i-1, 3*i, -1, 1)    # duplicado al original para la decision de formar la secuencia con este vertice
        add_edge(3*i, t, 0, 1)         # para terminar la secuencia
        ...
        if iequal_valid: add_edge(3*i-1, 3*ie-1, 0, inf)
